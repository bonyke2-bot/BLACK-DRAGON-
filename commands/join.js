...
async function joinCommand(sock, chatId, message) {
    try {
        ...
        await sock.sendMessage(chatId, { react: { text: "✅", key: message.key } });
        console.log(`[BONY-XMD] Successfully joined group: ${groupName}`);

    } catch (error) {
        console.error("[BONY-XMD] Join command error:", error);
