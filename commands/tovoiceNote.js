  ...
  try {
    ...
    if (!msg) {
      await sock.sendMessage(chatId, {
        text: "🎧 Reply to a *video* or *audio* to convert it to a voice note!"
    ...
    await sock.sendMessage(chatId, { text: "🔊 Converting to voice note..." },{ quoted: createFakeContact(message) });
    ...
    // Convert to PTT voice note (Opus in OGG container)
    await new Promise((resolve, reject) => {
      const command = ffmpeg(inputPath)
        .inputOptions('-t 59') // Limit duration to 59 seconds (WhatsApp voice note limit)
        .toFormat('ogg') // OGG container
        .audioCodec('libopus') // Opus codec for WhatsApp PTT
        .audioBitrate('64k') // WhatsApp voice note bitrate
        .audioChannels(1) // Mono for voice notes
        .audioFrequency(16000) // WhatsApp voice note frequency
        .on('start', (commandLine) => {
          console.log('FFmpeg voice note command:', commandLine);
        })
        .on('progress', (progress) => {
          if (progress.percent) {
            console.log(`Processing voice note: ${Math.round(progress.percent)}% done`);
          }
        })
        .on('end', () => {
          console.log('Voice note conversion completed successfully');
          resolve();
        })
        .on('error', (err) => {
          console.error('FFmpeg voice note error details:', {
            message: err.message,
            code: err.code,
            signal: err.signal
          });
      ...
      setTimeout(() => {
        if (command && command.ffmpegProc && !command.ffmpegProc.killed) {
          console.log('FFmpeg voice note timeout reached, killing process');
          command.kill('SIGKILL');
          reject(new Error('Voice note conversion timeout (60 seconds)'));
        }
      }, 60000); // 60 second timeout
    });

    // Read converted voice note file
    const audioStats = await fs.stat(outputPath);
    if (audioStats.size === 0) {
      throw new Error('Converted voice note file is empty');
    }

    const audioBuffer = await fs.readFile(outputPath);

    // Check file size limits (WhatsApp voice notes have smaller limits)
    const maxSize = 16 * 1024 * 1024; // 16MB max for WhatsApp
    if (audioBuffer.length > maxSize) {
      console.warn(`Voice note too large: ${(audioBuffer.length / 1024 / 1024).toFixed(2)}MB`);
      // For voice notes, we already limited duration, so file should be small
    }

    // Send as voice note (PTT = Push to Talk/voice message)
    await sock.sendMessage(chatId, {
      audio: audioBuffer,
      mimetype: 'audio/ogg; codecs=opus',
      ptt: true, // This makes it a voice note
      fileName: `voice_note_${timestamp}.opus`
    }, { quoted: createFakeContact(message) });

  } catch (err) {
    console.error("❌ toVoiceNote error:", {
      error: err.message,
      stack: err.stack,
      inputPath,
      outputPath
    ...
    let errorMessage = "💥 Failed to convert media to voice note.\n";
    ...
    if (err.message.includes('FFmpeg') || err.message.includes('conversion')) {
      ...
      errorMessage += "• File format is supported\n";
      errorMessage += "• Video is less than 59 seconds (voice note limit)";
