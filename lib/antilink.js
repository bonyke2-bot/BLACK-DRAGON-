...
const { getAntilink, incrementWarningCount, resetWarningCount } = require('../lib/index');
...
/**
 * Checks if a participant is a group admin.
 *
 * @param {string} jid - The group JID.
 * @param {string} participant - The participant JID.
 * @param {object} sock - The socket object.
 * @returns {Promise<boolean>} - True if the participant is an admin, otherwise false.
 */
async function isAdmin(jid, participant, sock) {
	try {
...
async function Antilink(msg, sock) {
	...
	const isSenderAdmin = await isAdmin(jid, sender, sock);
	if (isSenderAdmin) return; // Skip processing for admins
